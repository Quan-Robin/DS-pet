...
### DSH 审批直达（联动 DSH-desktop 伴生插件）
- DSH 等待工具审批时：鱼冒泡提醒 + 在鱼旁边弹出小窗（仅摘要），可直接 **批准 / 拒绝 / 看详情**
- 轮询桌面端伴生插件的 `/api/state`（`dsh_port` 可配，默认 3080）；未装插件或 DSH 未运行时静默
- 任一处（网页 / 桌面弹窗 / 桌宠）处理后，其余弹窗自动收起
- ⚠️ 弹窗只显示摘要——高危操作建议「看详情」后再决定；右键菜单「DSH → 审批提醒」可开关
